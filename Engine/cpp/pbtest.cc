...
#define K_ENC "encoding"
#define V_ENC "UTF-8"
#define K_VERSION "version"
#define V_VERSION "2013-07-28 22:00:28"


void build_menu(Menu::Country* country) {
	//Menu::Menu::Country* country = menu->add_countries();
	country->set_name("Sweden");
	Menu::City* city = country->add_cities();
	city->set_name("Gothenburg");
	Menu::Site* site = city->add_sites();
	site->set_name("Lindholmen");
	Menu::Restaurant* restaurant = site->add_restaurants();
	restaurant->set_name("Semcon");

	Menu::Dish* dish = restaurant->add_dishes();
	dish->set_name("Köttbullar");
	dish->set_desc("med lingon och potatismos");
	dish->set_price(75);

	dish = restaurant->add_dishes();
	dish->set_name("Flygande Jacob");
	dish->set_desc("med bananar och mycket kostigt");
	dish->set_price(76);
}
